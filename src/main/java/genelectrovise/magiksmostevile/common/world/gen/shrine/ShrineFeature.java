...
public class ShrineFeature extends Feature<IFeatureConfig> {
	...
	public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, IFeatureConfig config) {
		...
		if (!worldIn.isRemote()) {
			ServerWorld serverWorld = (ServerWorld) worldIn.getWorld().getServer().getWorld(worldIn.getDimension().getType());
			...
			try {
				...
				System.out.println("Generating Shrine using template at " + SHRINE_TEMPLATE);
				System.out.println("Template file : " + SHRINE_TEMPLATE.getPath());
				System.out.println("Namespace : " + SHRINE_TEMPLATE.getNamespace());

				System.out.println("TemplateManager attempting to get template from file " + SHRINE_TEMPLATE + " : " + manager.getTemplate(SHRINE_TEMPLATE));
